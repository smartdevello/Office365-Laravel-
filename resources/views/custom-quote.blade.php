...
{!!Form::open(['url'=>'save-custom-quote', 'id' => 'custom', 'enctype' => 'multipart/form-data', 'method'=>'post', 'class' => 'add_bottom_30']) !!}
...
<div class="container">
	...
	<h1>{{ isset($SurveyObject->first_name) ? $SurveyObject->first_name : base64_decode($ClientData->client) }} {{ isset($SurveyObject->last_name) ? $SurveyObject->last_name : base64_decode($ClientData->client_surname) }}</h1>
	<address><b>Pickup :</b> {{ base64_decode(@$ClientData->address) }}</address>
	<address><b>Delivery : </b> {{ base64_decode(@$ClientData->delivery_address) }}</address>
	
	<div class="row">
		<div class="col-md-12">
			<h3 style="display: none">Total Amount: £<span id="ta">{{ @$SurveyObject->total_price }}</span> </h3>
			...
			<input id="total_volume" name="total_volume" type="hidden" value="{{ @$SurveyObject->total_volume }}">
			<input id="total_amount" name="total_amount" type="hidden" value="{{ @$SurveyObject->total_price }}">
			
			<input id="txnnid" name="txnnid" type="hidden" value="{{ @$Quote->pay_txn_id }}">
			
			<input name="quote_id" type="hidden" value="{{ @$_GET['QuoteID'] }}">
			<input name="client_id" type="hidden" value="{{ $_GET['client'] }}">
			<input name="survey_id" type="hidden" value="{{ @$_GET['survey'] }}">
			...
			<fieldset title="Step 2" class="step" id="custom-step-1" style="display: block;">
				...
				<ul class="nav nav-tabs" id="myTab" role="tablist">
					...
					$TabCount = 0;
					@endphp
				@foreach($ItemList as $MainRoot => $ArrVal)
				  <li class="nav-item text-center">
					<a class="nav-link @if($TabCount == 0) active @endif" id="{{ $MainRoot }}-tab" data-toggle="tab" href="#{{ $MainRoot }}" role="tab" aria-controls="home" aria-selected="true" onClick="FilterRoomObject('{{ $MainRoot }}')">
						@if($MainRoot == "Bedroom")
						<i class="fa fa-bed" style="font-size: 30px;"></i><br><br>
						@endif
						
						@if($MainRoot == "Livingroom")
				...
				<div class="tab-content" id="myTabContent">
					...
					@endphp
					@foreach($ItemList as $MainRoot => $ArrVal)
				  		<div class="tab-pane fade @if($TabCount == 0) show active @endif" id="{{ $MainRoot }}" role="tabpanel" aria-labelledby="{{ $MainRoot }}-tab">
							<div class="row" id="AddExtraInfo-{{ $MainRoot }}">
						@foreach($ArrVal as $ItemName)
						@php
						...
						<div class="col-6">
							<div style="padding: 5px 10px; background: #FFF; font-size: 20px; font-weight: bold; border-bottom: #CCC 1px solid;">
								<div class="float-left">{{ $ItemName['Name'] }} <small>({{ $ItemName['Vol'] }})</small></div>
								<div class="float-right" id="ShowNoQtyBox-{{ $TotInvCounter }}" @if($CheckInventory != "") style="display: none" @endif><button onClick="QtyBoxShow('{{ $TotInvCounter }}')" type="button" class="btn btn-success">+</button></div>
								<div class="float-right" id="ShowQtyBox-{{ $TotInvCounter }}" @if($CheckInventory == "") style="display: none" @endif>
									<div class="float-left" style="margin-right: 10px;"><button type="button" onClick="DscQty('{{ $TotInvCounter }}')" class="btn btn-danger">-</button></div>
									<div class="float-left" style="margin-right: 10px;"><input data-item-id="item-{{ $MainRoot }}{{ $ItemName['ID'] }}" data-volume="{{ $ItemName['Vol'] }}" id="QtyBox-{{ $TotInvCounter }}" name="RoomObj[{{ $MainRoot }}][{{ $ItemName['ID'] }}]" value="{{ @$CheckInventory->obj_qty > 0 ? $CheckInventory->obj_qty : 0 }}" type="text" readonly class="form-control text-center VolumeBoxes" style="width: 50px;"></div>
									<div class="float-left"><button type="button" class="btn btn-success" onClick="IncQty('{{ $TotInvCounter }}')">+</button></div>
								</div>
								<div class="clearfix"></div>
							</div>
						</div>
...
<script src="{{ URL('public/typeahead.bundle.min.js	') }}"></script>
...
function createDownloadLink(blob) {
	...
	xhr.open("POST","{{ URL('UploadAudio') }}",true);
	xhr.send(fd);
}
</script>
