...
<div class="container">
	...
	<a href="{{ URL('MakeClientPayment') }}?SurveyID={{ @$_GET['SurveyID'] }}&QuoteID={{ @$_GET['QuoteID'] }}"><button class="btn btn-success">Confirm Your Booking Now</button></a>
	@endif
	...
	<ul class="nav nav-tabs">
		  ...
		  <li class="nav-item">
			<a class="nav-link @if($QuoteID == $AlQ->id) active @endif" href="{{ URL('ValidateClient') }}?SurveyID={{ $_GET['SurveyID'] }}&QuoteID={{ $AlQ->id }}">Plan {{ $QInd +1 }} - {{$AlQ->plan_name}}</a>
		  </li>
			@endif
			@endforeach
		</ul>
		
	{!!Form::open(['url'=>'change-status', 'enctype' => 'multipart/form-data', 'id' => 'myform', 'method'=>'post']) !!}
	<input type="hidden" name="qid" value="{{$Quote->id}}">
		<div class="clearfix">&nbsp;</div>
		<div class="text-center">
			Would you like to confirm this Personal Move Plan on the Live Calendar? &nbsp;&nbsp;
			<input onClick="$('#myform').submit()" value="1" name="JobConfimed" @if($Quote->client_confrim == "1") checked @endif type="radio"> Yes
			<input onClick="$('#myform').submit()" value="0" name="JobConfimed" @if($Quote->client_confrim == "0") checked @endif type="radio"> No
			<input onClick="$('#myform').submit()" value="2" name="JobConfimed" @if($Quote->client_confrim == "2") checked @endif type="radio"> Later
		</div>
	{!! Form::close() !!}
                        ...
                        <div class="row" style="margin-top:5px;">
                            ...
                            <div class="col-lg-4" style="background-color:#E9FFFF; border-left: #FFF 5px solid; border-right: #FFF 5px solid;">
                                <div class="row" style="margin:5px;">
                                    ...
                                    <div class="col-lg-6"><span style="font-size:14px;">
                                        {{ $MVJ->moving_job_arrival_from }} TO {{ $MVJ->moving_job_arrival_to }}
                                    </span></div>
                                </div>
                    ...
                    <div class="row" >
                        ...
                        <div class="col-lg-5">
                            <div class="text-white" style="font-size:60px;">
                               <b id="ShowTotalPrice">£{{ number_format($TotalPrice, 2) }}</b>
                            </div>    
                        </div>    
                    </div>    
                </div>    
