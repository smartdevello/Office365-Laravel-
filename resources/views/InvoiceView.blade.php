...
<body>
	...
	<div style="background-color: #2ACDC9; padding:5px 10px; text-align: center; color: #FFF">
		<div style="float: left"><b>No. :</b> CSR{{ date("dmY") }}{{ substr($Survey->last_name, 0, 1) }}</div>
	...
	<div style="padding: 10px;">
		<b>To</b><br>
		{{ $Survey->first_name." " . $Survey->last_name }}<br>
		{{ $Quote->pickup_postcode }}<br>
	</div>
		...
		<table width="100%" cellpadding="20" cellspaceing="0" border="1" style="border: 1px solid #dee2e6; border-collapse: collapse;">
			...
			<tbody>
				...
				<tr>
					...
					<td>£{{ number_format($Quote->client_paid_amount, 2) }}</td>
				</tr>
				@php
				...
				<tr>
					...
					<td>£{{ number_format($NetPrice, 2) }}</td>
					<td>£{{ number_format($TotalVAT, 2) }}</td>
					<td>£{{ number_format($TotlaPrice, 2) }}</td>
				</tr>
				<tr>
					<td colspan="3" style="text-align: right"><b>Total</b></td>
					<td width="20%"><b>£{{ $Quote->final_payable_amount }}</b></td>
				</tr>
			</tbody>
		</table>
